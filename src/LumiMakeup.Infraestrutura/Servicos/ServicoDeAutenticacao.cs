...
public sealed class ServicoDeAutenticacao : IServicoDeAutenticacao
{
    private readonly ContextoLumi _contexto;
    private readonly IPasswordHasher<Usuario> _passwordHasher;
    private readonly IServicoDeToken _servicoDeToken;
    private readonly IServicoDeAutenticacaoGoogle _servicoDeAutenticacaoGoogle;
    private readonly IValidadorDeRecaptcha _validadorDeRecaptcha;
    private readonly IServicoViaCep _servicoViaCep;
    private readonly IServicoNominatim _servicoNominatim;

    public ServicoDeAutenticacao(
        ContextoLumi contexto,
        IPasswordHasher<Usuario> passwordHasher,
        IServicoDeToken servicoDeToken,
        IServicoDeAutenticacaoGoogle servicoDeAutenticacaoGoogle,
        IValidadorDeRecaptcha validadorDeRecaptcha,
        IServicoViaCep servicoViaCep,
        IServicoNominatim servicoNominatim)
    {
        _contexto = contexto;
        _passwordHasher = passwordHasher;
        _servicoDeToken = servicoDeToken;
        _servicoDeAutenticacaoGoogle = servicoDeAutenticacaoGoogle;
        _validadorDeRecaptcha = validadorDeRecaptcha;
        _servicoViaCep = servicoViaCep;
        _servicoNominatim = servicoNominatim;
    }
    ...
    {
        var recaptchaValido = await _validadorDeRecaptcha.ValidarTokenAsync(requisicao.TokenRecaptcha ?? string.Empty, cancellationToken);
        if (!recaptchaValido)
        {
    ...
    {
        var dados = await _servicoDeAutenticacaoGoogle.ValidarTokenIdAsync(requisicao.TokenId, cancellationToken)
    ...
    {
        var usuarioId = _servicoDeToken.ObterIdDeUsuarioDoTokenRefresh(tokenRefresh);
    ...
    public async Task<UsuarioDto> CompletarPerfilAsync(long usuarioId, RequisicaoDeCompletarPerfil requisicao, CancellationToken cancellationToken = default)
    {
        ...
        if (requisicao.Endereco is not null)
        {
            ...
            var viaCep = await _servicoViaCep.ConsultarAsync(requisicao.Endereco.Cep, cancellationToken);
            ...
            var geo = await _servicoNominatim.GeocodificarAsync(enderecoCompleto, cancellationToken);
            if (geo.HasValue)
            {
    ...
    {
        var (tokenAcesso, tokenRefresh) = _servicoDeToken.GerarTokens(usuario);
